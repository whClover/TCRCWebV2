...
Public Class AssemblyPistonRec
    ...
    Sub load_progress()
        ...
        Dim query As String = "select dbo.PercentCalc(sum(case when Value is null then 0 else 1 end),
            count(IDEngineInput)) as PercComp from tbl_AssemblyEngineInput where wono=" & evar(ewo, 1) & " and CylinderDesc in('Recommendation','BoltRecom')"
        dt = GetDataTable(query)
        If dt.Rows.Count > 0 Then
    ...
    Sub load_data()
        ...
        Dim dt As New DataTable
        Dim query As String = "select * from v_AssemblyPistonRec where wono=" & evar(ewo, 1)
        dt = GetDataTable(query)
        If dt.Rows.Count > 0 Then
            rpt_PistonRec.DataSource = dt
            rpt_PistonRec.DataBind()
        End If
    End Sub

    Protected Sub rpt_PistonRec_ItemDataBound(sender As Object, e As RepeaterItemEventArgs)
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Dim ddBoldRec As DropDownList = CType(e.Item.FindControl("ddBoldRec"), DropDownList)

            Dim eCylinderNo As String = dataItem("CylinderNo").ToString()
            pNo.InnerText = "#" & eCylinderNo
            ddRec.SelectedValue = dataItem("Recommendation").ToString()
            ddBoldRec.SelectedValue = dataItem("BoltRecom").ToString()
        End If
    End Sub

    Protected Sub rpt_PistonRec_ItemCommand(source As Object, e As RepeaterCommandEventArgs)
        If e.CommandName = "Save" Then
            Dim cylinderNo As String = Convert.ToString(e.CommandArgument)
            Dim ddRec As DropDownList = CType(e.Item.FindControl("ddRec"), DropDownList)
            Dim ddBoldRec As DropDownList = CType(e.Item.FindControl("ddBoldRec"), DropDownList)
            Dim query_1 As String = "update tbl_AssemblyEngineInput set Value=" & evar(ddRec.SelectedValue, 1) & ",ModDate=GetDate(),ModBy=" & eByName() & " 
                                    where wono=" & evar(ewo, 1) & " and CylinderDesc='Recommendation' and CylinderNo=" & evar(cylinderNo, 1)
            Dim query_2 As String = "update tbl_AssemblyEngineInput set Value=" & evar(ddBoldRec.SelectedValue, 1) & ",ModDate=GetDate(),ModBy=" & eByName() & " 
                                    where wono=" & evar(ewo, 1) & " and CylinderDesc='BoltRecom' and CylinderNo=" & evar(cylinderNo, 1)
    ...
    Protected Sub bna_Click(sender As Object, e As EventArgs)
        ...
        query = "update tbl_AssemblyEngineInput set Value='n/a',ModBy=" & eByName() & ",ModDate=GetDate() where wono=" & evar(ewono, 1) & " and CylinderDesc in('Recommendation','BoltRecom')"
        executeQuery(query)
